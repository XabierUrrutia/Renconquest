...
public class BuildingManager : MonoBehaviour
{
    ...
    [Header("Base e �rea de constru��o")]
    public Transform militaryBase;
    [Tooltip("Raio onde � permitida a constru��o (unidades do mundo)")]
    public float buildRadius = 10f;

    [Header("Layers e valida��o")]
    [Tooltip("Layers que bloqueiam a coloca��o (ex.: Ground, Buildings)")]
    ...
    [Header("Constru��o - Animator")]
    [Tooltip("Nome do par�metro bool no Animator para marcar 'under construction' (se existir)")]
    public string animatorUnderConstructionBool = "UnderConstruction";
    [Tooltip("Pulso fallback (se n�o houver Animator)")]
    ...
    void Awake()
    {
        ...
        {
            Debug.LogWarning($"BuildingManager: inst�ncia duplicada em '{name}', destruindo componente.");
            Destroy(this);
            return;
        }
    }
    ...
    void Update()
    {
        ...
        if (previewInstance != null)
        {
            ...
            // Validar posici�n (Rojo/Verde)
            bool valid = IsValidPlacement(targetPos, out Collider2D[] overlapping);
            UpdatePreviewVisual(valid);

            // --- 2. CONFIRMAR CONSTRUCCI�N (CLIC IZQUIERDO) ---
            if (Input.GetMouseButtonDown(0)) // 0 es bot�n izquierdo
            {
    ...
    public void SelectBuilding(BuildingData buildingData)
    {
        ...
        {
            Debug.LogWarning($"BuildingManager: prefab n�o definido no BuildingData '{buildingData.buildingName}'.");
            return;
        }
    ...
    public void StartPlacing(BuildingData data)
    {
        ...
        Debug.Log($"BuildingManager: Modo construcci�n activado para '{data.buildingName}'.");
    }
    ...
    public bool TrySpendResources(int amount)
    {
        ...
        {
            Debug.LogWarning("[BuildingManager] MoneyManager n�o encontrado. Permitindo constru��o sem custo.");
            return true;
        }
    }
    ...
    {
        // Guardar escala original ANTES de cualquier modificaci�n
    ...
    bool IsValidPlacement(Vector3 worldPos, out Collider2D[] overlapping)
    {
        ...
        // 4. No encima de edificios, jugadores ni f�bricas enemigas
